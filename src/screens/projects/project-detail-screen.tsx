...
export function ProjectDetailScreen({ projectId }: { projectId: string }) {
  const { currentProject, fetchProjectDetail, createMission, createHandoff, launchRole, updateProject, fetchRecommendation, recommendation } = usePaperclipStore() as any
  useEffect(() => {
      ...
      <div className="grid gap-4 xl:grid-cols-[1.2fr_2fr]">
        <div className="space-y-4">
          <PaperclipCard className="space-y-3">
            ...
            {recommendation ? <div className="space-y-2 border-2 border-dashed border-[var(--theme-border)] p-3"><div className="brutalist-label">Recommended next action</div><div className="font-semibold text-[var(--theme-text)]">{recommendation.action}</div><p className="text-sm text-[var(--theme-muted)]">{recommendation.rationale}</p></div> : null}
          </PaperclipCard>
